...
namespace AesAlgorithm
{
    class AesImplementation
    {
        ...
        private static byte[] XorOperation(byte[] a, byte[] b)
        {
        ...
        private static byte[][] GenerateSubkeys(byte[] key)
        {
            ...
            while (i < _nb * (_nr + 1))
            {
                ...
                tmp[i] = XorOperation(tmp[i - _nk], temp);
                i++;
            }
        ...
        private static byte[] SubWord(byte[] inBytes)
        {
            ...
            for (int i = 0; i < tmp.Length; i++)
                tmp[i] = (byte)(Sbox[inBytes[i] & 0x000000ff] & 0xff);
        ...
        private static byte[][] InvSubBytes(byte[][] state)
        {
            for (int row = 0; row < 4; row++)
                for (int col = 0; col < _nb; col++)
                    state[row][col] = (byte)(InverseSbox[(state[row][col] & 0x000000ff)] & 0xff);
        ...
        public static byte[] Encrypt(byte[] inBytes, byte[] key)
        {
            ...
            byte[] tmp = new byte[inBytes.Length + length];
